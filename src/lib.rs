...
where
    T: num_traits::Float + std::fmt::Debug + fitsimg::TypeToImageType + fitsio::images::WriteImage,
{
    ...
    T: num_traits::Float + std::fmt::Debug + fitsimg::TypeToImageType + fitsio::images::WriteImage,
{
    ...
    let wd = |n: isize, m: isize| -> T {
        (-(T::from(n * n)).unwrap() / (two * sigma_y.powi(2))
            - (T::from(m * m)).unwrap() / (two * sigma_x.powi(2)))
        .exp()
    };
    let mut vp = Array2::zeros((v.nrows() + kernel_y, v.ncols() + kernel_x));

    vp.slice_mut(s![
        kernel_y / 2..vp.nrows() - kernel_y / 2,
        kernel_x / 2..vp.ncols() - kernel_x / 2
    ])
    .assign(&v);

    let mut wfp = Array2::zeros((v.nrows() + kernel_y, v.ncols() + kernel_x));
    wfp.slice_mut(s![
        kernel_y / 2..wfp.nrows() - kernel_y / 2,
        kernel_x / 2..wfp.ncols() - kernel_x / 2
    ])
    .assign(&mask.map(|&x| match x {
        FlagState::Flagged => zero(),
        FlagState::Normal => one(),
    }));
    let kernel_0 = (-(kernel_y as isize) / 2..=kernel_y as isize / 2)
        .map(|n| wd(n, 0))
        .collect::<Array1<T>>();
    let kernel_1 = (-(kernel_x as isize) / 2..=kernel_x as isize / 2)
        .map(|m| wd(0, m))
        .collect::<Array1<T>>();
    println!("{:?}", kernel_1);
    let vh = run_gaussian_filter(
        &vp,
        v.nrows(),
        v.ncols(),
        wfp,
    ...
    let vh = vh
        .slice(s![
            kernel_y / 2..vh.nrows() - kernel_y / 2,
            kernel_x / 2..vh.ncols() - kernel_x / 2
        ])
        .to_owned();
    ...
    for i in ny2..vs0 + ny2 {
        for j in nx2..vs1 + nx2 {
            match mask[(i - ny2, j - nx2)] {
                ...
                _ => {
                    let val = (&wfp.slice(s![i - ny2..=i + ny2, j])
                        * &vp.slice(s![i - ny2..=i + ny2, j])
                        * &kernel_0)
                        .sum();
    ...
    for j in nx2..vs1 + nx2 {
        for i in ny2..vs0 + ny2 {
            match mask[(i - ny2, j - nx2)] {
                ...
                _ => {
                    let val = (&wfp.slice(s![i, j - nx2..=j + nx2])
                        * &vh.slice(s![i, j - nx2..=j + nx2])
                        * &kernel_1)
                        .sum();
...
pub fn binary_mask_dialation(
    ...
    let mut img = ImageBuffer::<Luma<u8>, Vec<u8>>::from_raw(
        width as u32,
        height as u32,
        mask.into_shape((width * height,))
            .unwrap()
            .to_vec()
            .into_iter()
            .map(|x| match x {
    ...
    T: num_traits::Float + std::fmt::Debug + fitsimg::TypeToImageType + fitsio::images::WriteImage,
{
    for i in 0..data.nrows() {
        let r: Vec<_> = data
            ...
            .iter()
            .zip(mask.row(i).iter())
            .filter(|(&_a, &b)| b == FlagState::Normal)
            .map(|(&a, _b)| a)
            .collect();
        let med = median(&r);
        data.row_mut(i)
            .iter_mut()
            .for_each(|x| *x = (*x - med).abs());
    }
    data
}

pub fn _sumthreshold<T>(
    data: &ArrayView2<T>,
    mask: Array2<FlagState>,
    i: usize,
    chi: T,
...
pub fn _run_sumthreshold<T>(
    data: &Array2<T>,
    init_mask: &Array2<FlagState>,
    eta: T,
    M: &[usize],
    chi_i: &[T],
    kernel_m: usize,
    ...
    T: num_traits::Float + std::fmt::Debug + fitsimg::TypeToImageType + fitsio::images::WriteImage,
{
    ...
    for (&m, &chi) in M.iter().zip(chi_i.iter()) {
        ...
        if m == 1 {
            ...
            panic!();
            st_mask = _sumthreshold(&res.t(), st_mask.t().to_owned(), m, chi)
                .t()
                .to_owned();
...
pub fn get_rfi_mask<T>(
    data1: ArrayView2<T>,
    mask1: Option<ArrayView2<FlagState>>,
    chi_1: T,
    eta_i: &[T],
    normalize_standing_wave: bool,
    suppress_dialation: bool,
    kernel_m: usize,
    kernel_n: usize,
    sigma_m: T,
    sigma_n: T,
...
where
    T: num_traits::Float + std::fmt::Debug + fitsimg::TypeToImageType + fitsio::images::WriteImage,
{
    let mask = mask1.map_or(
        Array2::<FlagState>::default((data1.nrows(), data1.ncols())),
        |x| x.to_owned(),
    );
    ...
    let chi_i: Vec<_> = M
        .iter()
        .map(|&m| chi_1 / p.powf(T::from(m).unwrap().log2()))
        .collect();
    let mut st_mask = mask;
    for &eta in eta_i {
        st_mask = _run_sumthreshold(
            &data,
            &st_mask,
            eta,
            &M[..],
    ...
    if suppress_dialation {
        ...
        binary_mask_dialation(
            mask1.map_or(st_mask.clone(), |x| !((&x) ^ (&st_mask))),
            di_args.0,
            di_args.1,
            2,
        ) | st_mask
...
pub fn get_rfi_mask_py(
    data1: ArrayView2<f64>,
    mask1: ArrayView2<bool>,
    chi_1: f64,
    eta_i: &[f64],
    normalize_standing_wave: bool,
    suppress_dialation: bool,
    ...
    let result = get_rfi_mask(
        data1,
        Some(mask1.view()),
        chi_1,
        eta_i,
        normalize_standing_wave,
        suppress_dialation,
        kernel_m,
        kernel_n,
        sigma_m,
...
fn native(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    fn get_csmat_py(
        ...
        get_rfi_mask_py(
            ...
            mask1.as_array(),
            chi_1,
            eta_i.as_array().to_owned().as_slice().unwrap(),
            normalize_standing_wave,
            suppress_dialation,
            kernel_m,
            kernel_n,
